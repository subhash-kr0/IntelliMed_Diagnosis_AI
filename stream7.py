...
#     # Display from original inputs_dict for user-friendliness
#     for k, v in result_data['inputs'].items():
#         # Get full form name if available
...
#     pdf.set_fill_color(230, 230, 230) # Light grey for header background
#     pdf.set_font("Arial", 'B', 16)
#     pdf.cell(0, 10, txt="Medical Diagnosis Report", ln=True, align='C', fill=True)
#     pdf.set_font("Arial", size=10)
#     pdf.cell(0, 7, txt=f"Diagnosed by {APP_NAME} (Version {APP_VERSION})", ln=True, align='C')
#     pdf.cell(0, 7, txt=f"Report Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True, align='C')
#     pdf.ln(10)

#     pdf.set_font("Arial", 'B', 12)
#     pdf.cell(0, 8, txt=f"Disease Assessed: {result_data['title']}", ln=True)
#     pdf.set_font("Arial", '', 12)
#     pdf.cell(0, 8, txt=f"Diagnosis Result: {result_data['diagnosis']}", ln=True)
#     pdf.cell(0, 8, txt=f"Confidence Score: {result_data['confidence']}", ln=True)
#     pdf.ln(6)

#     pdf.set_font("Arial", 'B', 12)
#     pdf.cell(0, 8, txt="Patient Provided Details:", ln=True)
#     pdf.set_font("Arial", '', 10)
#     for k, v in result_data['inputs'].items():
#         label_k_pdf = feature_fullforms.get(result_data['title'], {}).get(k, k)
#         pdf.multi_cell(0, 7, txt=f"{label_k_pdf}: {v}", border=0) # Use multi_cell for longer values
#     pdf.ln(5)
...
#     try:
#         pdf_bytes_output = pdf.output(dest='S').encode('UTF-8') # FPDF output is latin-1
#         pdf_buffer = io.BytesIO(pdf_bytes_output)
#         pdf_buffer.seek(0)
