...
with st.sidebar:
    ...
    HUGGINGFACE_REPO_ID = st.selectbox(
        "Select Model",
        ["mistralai/Mistral-7B-Instruct-v0.3", "HuggingFaceH4/zephyr-7b-beta", "HuggingFaceH4/zephyr-9b-beta", "mistralai/Mixtral-8x7B-Instruct-v0.1"],
        index=0
    )
...
#     st.markdown("---")
#     st.subheader("About")
#     st.markdown("🤖 **AI Chatbot** powered by **LangChain** & **Mistral AI**.")


# # Mistral-compatible LangChain LLM wrapper
# class MistralLangChainLLM(LLM):
#     def __init__(self, api_key: str, model: str = "mistral-small", temperature: float = 0.5):
...
#     def generate_prompt(self, prompt: str) -> LLMResult:
...
# def load_llm(repo_id, mistral_api_key, temperature):
#     """Load Mistral LLM."""
#     try:
#         return MistralLangChainLLM(
#             api_key=mistral_api_key,
#             model=repo_id,
#             temperature=temperature
#         )
#     except Exception as e:
#         st.error(f"❌ Error loading Mistral LLM: {e}")
#         return None
...
#             st.error("❌ Mistral API key not found. Set MISTRAL_API_KEY in environment variables.")
#             return
...
#         try:
#             llm_model = load_llm(mistral_model, mistral_api_key, response_temp)
#             if llm_model is None:
#                 return  # Error already displayed
